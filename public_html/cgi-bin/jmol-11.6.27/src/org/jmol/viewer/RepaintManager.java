/* $RCSfile$
 * $Author: hansonr $
 * $Date: 2009-05-03 14:55:44 +0200 (Sun, 03 May 2009) $
 * $Revision: 10842 $
 *
...
class RepaintManager {
  ...
  void popHoldRepaint() {
    --holdRepaint;
    //System.out.println("applet test RepaintManager.popholdrepaint() holdRepaint=" + holdRepaint+ " " + Thread.currentThread().getName() + " " + Thread.currentThread());
    if (holdRepaint <= 0) {
      holdRepaint = 0;
      repaintPending = true;
      //System.out.println("applet test RepaintManager.popHoldRepaint()-->Viewer.repaint() ");
      viewer.repaint();
    }
  }

  boolean refresh() {
    //System.out.println("applet test RepaintManager.refresh() repaintPending=" + repaintPending + " holdRepaint = " + holdRepaint+ " " + Thread.currentThread().getName() + " " + Thread.currentThread());
    if (repaintPending)
      return false;
    repaintPending = true;
    if (holdRepaint == 0) {
      //System.out.println("applet test RepaintManager.refresh()-->Viewer.repaint()");
      viewer.repaint();
    }
    return true;
  }
  ...
  synchronized void requestRepaintAndWait() {
    //System.out.println("applet test RepaintManager.RepaintAndWait()-->Viewer.repaint()"+ " " + Thread.currentThread().getName() + " " + Thread.currentThread());
    viewer.repaint();
    try {
      wait();
  ...
  synchronized void repaintDone() {
    ...
    //System.out.println("applet test RepaintManager.repaintDone()-->notify() " + Thread.currentThread().getName() + " " + Thread.currentThread());
