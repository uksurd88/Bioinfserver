	...
	if (keycheck==2) var gocheck = (gokey1+gokey2);
	 ...
	 if (eventchecker>0) firsttimeselect=1; // to remain in sheet globally
		...
		if (typeof(currentcll)!="undefined") {$(currentcll).style.border = "1px solid white";}
			cellid(e);
			//debugInfo("coming in"); ALERT Alternative.
			...
			querycurrentnumber = parseInt(querycurrentnumber);
				...
				display = "I am your Virtual Navigator";
				 if (key=="27") {$(currentcell).style.border = "1px solid white";$(nextcell).style.border = "1px solid white";firsttimeselect=0;currentcell=previouscell="";$(currentcell).blur();}
				 ...
				 //aif (key =="114") {resetsheet();display = "Sheet Resetted";}  	   // This resets the sheet, assigned for 'r'.
				 ...
				 log.debug(display);
				 if (yn == "y") {
					new Ajax.Updater("navigation",'http://bioinf3.bioc.le.ac.uk/~ss533/cgi-bin/sheetnavigation.cgi',{
				...
				if (currentcellid == "name") {
					if (key != "39") {
					file = 'http://bioinf3.bioc.le.ac.uk/~ss533/cgi-bin/migrationsessionshow.cgi';
					sessionname =$(nextcell).innerHTML;
					date ="none";
					}
					if (key == "39"){
					file = 'http://bioinf3.bioc.le.ac.uk/~ss533/cgi-bin/migrationdateshow.cgi';
				...
				if (currentcellid == "date") {
					...
					if (key == "39") {
					file = 'http://bioinf3.bioc.le.ac.uk/~ss533/cgi-bin/migrationmoduleshow.cgi';
					sessionname = $("name"+currentcellidnumber).innerHTML;
					date = "none";
				...
				if (currentcellid == "mode") {
					...
					file ='http://bioinf3.bioc.le.ac.uk/~ss533/cgi-bin/sessions2.cgi';
					date = "none";
					module = "none";
					}
					if (key == "37") {
				...
				if (currentcellid == "qvry") {
					...
					module = $("mode"+currentcellidnumber).innerHTML;
						...
						parameters:{session:sessionname,date:date,module:module,query:userquery}});
					 	 $(nextcell).style.border = "7px inset SteelBlue";
