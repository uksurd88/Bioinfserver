...
function usercol() {
	...
	topII.insert({Bottom:'<td><input type="text"  onChange="usercolprocessor(0);" id="usercol'+colcount+'" value="         User Column"></input>'});
		...
		rowII.insert({Bottom:item});
			...
			var meanrow=$("MeanRow");
			var itemm='<td id="mean-'+colcount+'"><input type="hidden" id="usermean"></input></td>';
			meanrow.insert({Bottom:itemm});
				var stdevrow=$("stdevrow");
				var rowstdev='<td id="stdv-'+colcount+'"><input type="hidden" id="userstdev"></input></td>';
				stdevrow.insert({Bottom:rowstdev});
}
...
if (null!=previouscell) {$(previouscell).style.border = "1px solid white";}
//if (typeof(previouscell)!="undefined") alert(previouscell);
$("userquery"+columnnumber).blur();
currentcll=("qvry"+0+'_'+columnnumber);
$(currentcll).style.border = "7px inset SteelBlue";
currentcell=currentcll;
//nextcell=currentcell;
//$("qvry"+0+'_'+columnnumber).focus();
firsttimeselect=1;
var query=$("userquery"+columnnumber).value;
new Ajax.Request('http://bioinf3.bioc.le.ac.uk/~ss533/cgi-bin/ask2.cgi', {
	method : 'get',
...
var uservariable = $("usercol"+colcount).value;
	...
	var uservalue = $("usercol"+userrowNumber+'_'+colcount).value;
		new Ajax.Updater('','http://bioinf3.bioc.le.ac.uk/~ss533/cgi-bin/usercolprocessor.cgi',{
